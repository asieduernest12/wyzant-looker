...
export default function Lessons(/** @type Element */ container_element, /** @type string */ url) {
  ...
  function renderEarningStats() {
    ...
    const avg_tutored = parseFloat(`${stat_values.map(({ tutored_minutes }) => tutored_minutes).reduce((acc, tutored_minutes) => acc + tutored_minutes) / stat_values.length}`).toFixed(2);
  ...
  function makeDateStat(day) {
    /** @lends */
    return {
      earned: 0, tutored_minutes: 0, payed: day.date.includes('*'), date: day.date.replace('*', ''),
    };
  }
  ...
  function makeLesson(/** @type Element */ lesson_row_el) {
    const PROPS = {
      Date: 0, Length: 1, Entered: 2, Online: 3, Student: 4, Subject: 5, Rating: 6, Rate: 7, Pay: 8, Earned: 9, Mileage: 10, Payment: 11, Status: 12,
    };
