...
export default function Lessons(/**@type Element*/ container_element, /**@type string */ url) {
	...
	function renderEarningStats() {
		...
		let lessons = [...document.querySelectorAll('[id="LessonsList"] tbody tr').values()].map(makeLesson)

		let stat_values = [...getDateStats(lessons).values()]

		let avg_earned = parseFloat(stat_values.map(({ earned }) => earned).reduce((acc, earned) => acc + earned) / stat_values.length + "").toFixed(2)
		let avg_tutored = parseFloat(stat_values.map(({ tutored_minutes }) => tutored_minutes).reduce((acc, tutored_minutes) => acc + tutored_minutes) / stat_values.length + "").toFixed(2)

		let stats_row_html = stat_values.map(makeDateStatElement).join("")

		let stats_table_html = `
	...
	function getDateStats(lessons) {
		let /**@type Map<string,LessonEntry>*/ daily_stats = new Map()

		for (let day of lessons) {
			let /**@type {DateStat} */ date_stat = daily_stats.get(day.date.replace("*", "")) ?? makeDateStat(day)
	...
	function makeDateStat(day) {
		/**@lends */
		return { earned: 0, tutored_minutes: 0, payed: day.date.includes("*"), date: day.date.replace("*", "") }
	}
