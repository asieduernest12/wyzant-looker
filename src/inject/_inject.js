...
function getStudentRatePromise(selectedStudent) {
  ...
  return RateFinder()
    ...
    .then(rendererFn, () =>
      rendererFn({ name: "", rate_online: "NA", rate_inperson: "NA" })
    );
}
...
function addIFrame() {
  ...
  let /** @type HTMLIFrameElement */ wzlIfram =
      document.querySelector("#wzl_iframe");
  ...
  if (isNew) {
    ...
    wzlIfram.setAttribute(
      "src",
      "https://www.wyzant.com/tutor/jobapplication/history?sort=1&pagenumber=0&pagesize=100"
    );
...
function showWzlIndicator() {
  ...
  readyIndicator.setAttribute(
    "tooltip",
    "Click to enable profile lookup of students (underlined)"
  );
...
function activateWzlIndicator() {
  document
    .querySelector(".wzl_indicator")
    .classList.add("wzl_indicator_active");
}
function deActivateWzlIndicator() {
  document
    .querySelector(".wzl_indicator")
    .classList.remove("wzl_indicator_active");
}
...
function findStudentResult(iframe, studentFirstName) {
  ...
  const jobNodes =
    iframe.contentWindow.document.querySelectorAll(".job-result");
  const jobs = Array.from(jobNodes);
  const firstFound = jobs.find(job =>
    job.textContent.includes(studentFirstName)
  );
  return firstFound;
}
...
function closeWzlCard(reset) {
  ...
  if (reset ?? false) {
    ...
    document
      .querySelector(".show-more a.btn")
      .removeEventListener("click", showMoreConvoBtnClick);
...
const RenderJobPostDetails = studentResultInfo => {
  ...
  localWzlProfile.innerHTML = makeWzlStudentCard(
    makeStudentAdapter(studentResultInfo)
  );

  // add close and reset listeners
  document
    .querySelector(".wzl-card--close")
    .addEventListener("click", () => closeWzlCard(false));
  document
    .querySelector(".wzl-card--reset")
    .addEventListener("click", () => closeWzlCard(true));
  console.log("%c job post details ready", "background:lightyellow");
};

function getStudentJobPostPromise(
  /** @type Boolean */ isIframeNew,
...
function messageDocumentClickHandler(event) {
  ...
  Promise.all([
    getStudentJobPostPromise(isNew, iframe, studentFirstName),
    getStudentRatePromise(selectedStudent)
  ]).catch(() => alert("Error while fetching job info and hourly rate info"));
}
...
function wzlConvoClickHandler() {
  ...
  const convos = Array.from(
    document.querySelectorAll(".conversation-summary-wrap")
  ).filter(convo => !convo.classList.contains("wzl-listener-active"));
  if (convos.length) {
  ...
  document
    .querySelector(".show-more a.btn")
    .addEventListener("click", showMoreConvoBtnClick);
...
function init() {
  ...
  const LOCATION_TRIGGER_TOKENS = {
    ...
    statistics_page: {
      ...
      action: () => {
        ...
        const isStatTable = /your stats/i.test(
          statTable.closest(".row").querySelector(".gray-header").textContent
        );
        ...
        const loggable = statTableRows.map((tr, index) => {
          ...
          if (index === 0) {
            ...
            [hrsDaily, $daily, $hourly, daysTd] = ["th", "th", "th", "th"].map(
            ...
            [hrsDaily, $daily, $hourly, daysTd] = ["td", "td", "td", "td"].map(
              i => document.createElement(i)
            );
            daysCount = {
            ...
            totalHours = Number(
              tr.querySelectorAll("td")[1]?.textContent?.replace(/,/, "")
            );

            hrsDaily.textContent =
              Number(totalHours / daysCount).toFixed(2) + "";
    ...
    submitLessonReview: {
      ...
      action: () => {
        ...
        const lessonReportTextField = /** @type {HTMLTextAreaElement} */ (
          document.querySelector("textarea[name=LessonReview][id=LessonReview]")
        );
        const studentName = document
          .querySelector('a[href^="/tutor/students/view?suid"]')
          ?.textContent?.replace(/\n|\W{3,}/g, "");
        const studentSubject = /** @type {HTMLSelectElement} */ (
          document.querySelector("select[name=SubjectID][id=SubjectID]")
        )?.selectedOptions[0]?.text?.replace(/\n|\W{3,}/g, "");

        if (
          [studentName, studentSubject, lessonReportTextField].some(x => !x)
        ) {
        ...
        const processedTemplate = ("" + lessonReviewSubmissionTemplateText)
          .replaceAll("[name]", studentName)
          .replaceAll("[subject]", studentSubject);
        ...
        const observer = new MutationObserver(mutationsList => {
          const isMutationTypeChildList = mutationsList.some(
            mutation => mutation.type === "childList"
          );

          if (!isMutationTypeChildList) return;

          const confirmLessonInput = document.querySelector(
            "input[name=confirmLesson]"
          );
  ...
  const [key, pageItem] = Object.entries(LOCATION_TRIGGER_TOKENS).find(
    ([, { test }]) => test.test(pageUrl)
  );
